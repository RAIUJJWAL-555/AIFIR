...
const getAllComplaints = asyncHandler(async (req, res) => {
  ...
  const complaints = await Complaint.find().populate('user', 'name email phone').sort({ createdAt: -1 });
...
const createComplaint = asyncHandler(async (req, res) => {
  const { title, description, incidentType, location, incidentDate, incidentTime, aiDraft } = req.body;
  ...
  const complaint = await Complaint.create({
    user: req.user.id,
    onModel: req.user.role === 'citizen' ? 'Citizen' : 'User',
    title,
    description,
    incidentType,
    location,
